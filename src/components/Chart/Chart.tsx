...
export default function Chart({ dataType, selectedWeek, allWeeksData }: ChartProps) {
  ...
  const currentWeekData = allWeeksData.find((week) => week.week === selectedWeek)
  ...
  useEffect(() => {
    ...
    const hasDataChanged = JSON.stringify(previousDataRef.current) !== JSON.stringify(currentData)

    if (hasDataChanged) {
      // Preserve current animated values as start values for smooth transition
      const newAnimatedData = currentData.map((item, index) => {
        const currentAnimatedValue = animatedData[index]?.currentValue ?? item.value
        return {
          day: item.day,
  ...
  const drawChart = () => {
    ...
    const dataPoints = animatedData.map((data, index) => ({
      ...
      y: paddingTop + chartHeight - ((data.currentValue - minValue) / valueRange) * chartHeight,
    ...
    dataPoints.forEach((point, index) => {
      ...
      const pulseRadius = isAnimating ? baseRadius + Math.sin(performance.now() * 0.005) * 0.5 : baseRadius
  ...
  }, [animatedData, allIncomeData, allExpenseData]) // Updated dependencies
